...
export function PromptForm({ prompt, onSave, onCancel }: PromptFormProps) {
  ...
  const validateForm = async () => {
    ...
    if (!formData.key.trim()) {
      ...
      const isUnique = await storageService.isPromptKeyUnique(formData.key, prompt?.id)
      if (!isUnique) {
        ...
        <div className="plasmo-p-6">
          ...
          <form onSubmit={handleSubmit} className="plasmo-space-y-4">
            ...
            <div>
              ...
              />
              {errors.key && (
                <p className="plasmo-text-red-500 plasmo-text-xs plasmo-mt-1">{errors.key}</p>
              )}
            ...
            <div>
              ...
              />
              {errors.title && (
                <p className="plasmo-text-red-500 plasmo-text-xs plasmo-mt-1">{errors.title}</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.content && (
                <p className="plasmo-text-red-500 plasmo-text-xs plasmo-mt-1">{errors.content}</p>
              )}
            </div>
